...
function isMonsterHit() {
    return Math.random() > .2 || health < 20; // 80% chance to hit, but if health is below 20, always hit
    // this is to prevent the player from losing the game due to a string of bad luck
}

function getMonsterAttackValue(level) {
    const hit = (level * 5) - (Math.floor(Math.random() * xp));
    console.log(hit);
    return hit > 0 ? hit : 0; // there's a bug that makes the monster attack do negative damage, so we fix it here
    // by returning 0 if the hit is negative
}
